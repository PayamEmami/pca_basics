...
Matrix multiplication is used to transform the space into another shape. This multiplication is defined between two matrices and will produce another matrix as the result. In order to be able to multiply two matrices, the number of columns of the first matrix (left-hand side) **MUST** matches the number of rows in the second matrix. If the dimensions of the first matrix are $l \times m$ and the dimensions of the second matrix is $m \times n$, the result of the multiplication will be a matrix with dimensions of $l \times n$.
...
Ok! That became a little boring. To solve this, we could just find the inverse of $(A -\lambda \cdot I_n)$ and multiply it by both sides? We know that if we multiply a matrix by its inverse, the result will be an identity matrix (like 1 in you divide a number by itself). In this case, $v$ will be zero. So our eigenvector is just zero! Boring! 
...
We can calculate the covariance matrix of $A$ simply by doing $Cov(A)=\frac{1}{n-1} \times A^T \cdot A$ (look the matrix multiplication).  Look at covariance formula, the first part of $Cov(A)=\frac{1}{n-1} \times A^T \cdot A$ is $\frac{1}{n-1}$ which is basically the division we do in $Cov(x,y)=\frac{\sum_{i=1}^{N}(x_{i}-\bar{x})(y_{i}-\bar{y})}{n-1}$. The second part is doing $Cov(x,y)=\sum_{i=1}^{N}(x_{i})(y_{i})$ we are missing something! What is that? Subtraction by the average. Here comes the trick. If we [center our data](#centerExample), the average of each variable becomes zero. So  $Cov(x,y)=\frac{\sum_{i=1}^{N}(x_{i}-\bar{x})(y_{i}-\bar{y})}{n-1}$ becomes $Cov(x,y)=\frac{\sum_{i=1}^{N}(x_{i}-0)(y_{i}-0)}{n-1}$ that is exactly $Cov(A)=\frac{1}{n-1} \times A^T \cdot A$. There are some functions, software that do covaraince matrix using the tradition formula however, to be safe, make sure that you **ALWAYS** center your data. In the rest of this chapter, we assume that we have centered our data. 
...
Let's say our data is sitting in matrix $X$, in which the rows are observations and the columns are variables. Our goal is to find an orthonormal matrix $P$ that when we mapped map our original data $X$ to another space $Y$ so that the covariance of $Y$ becomes diagonal (zero on all off-diagonal elements). So the goal is $Y=X \cdot P$ so that $Cov(Y)=diagonal \ or \ \frac{1}{n}Y^T \cdot Y=diagonal$. 

So as said before we can calculate the covariance of the mapped data ($Y$) as $Cov(Y)= \frac{1}{n}Y \cdot Y^T$. We covariance of the original data (before transformation) is $Cov(X)= \frac{1}{n}X^T \cdot X$  We can try to rewrite this formula like this:


$Cov(Y)= \frac{1}{n}Y^T \cdot Y$
Did we say that $Y=XP$ right?
So let's replace $Y$ by $XP$ in right side of $Cov(Y)= \frac{1}{n}Y^T \cdot Y$

$Cov(Y)= \frac{1}{n} (XP)^T \cdot (XP)$
...
Great! Do you see something cool in the middle of the equation? Yes. That is the covariance of the original matrix ($Cov(X)= \frac{1}{n}X^T \cdot X$). 
...
$Cov(X)=V \Lambda  \cdot V^T$. 

We said that $Cov(X)=\frac{1}{n}X^TX$ so then $Cov(X)=V \Lambda  \cdot V^T$ becomes  $\frac{1}{n}XX^T=V \Lambda  \cdot V^T$
...
We said that $\frac{1}{n}XX^T=V \Lambda  \cdot V^T$ so why not replace $\frac{1}{n}X^TX$ in $Cov(Y)=  P^T(\frac{1}{n}X^T X)P$ by $V \Lambda  \cdot V^T$? Then we get rid of $X$. 
Let's do that

$Cov(Y)=  P^T(V \Lambda  \cdot V^T)P$
...
$Cov(Y)=  V^T(V \Lambda  \cdot V^T)V$

We said the $V^T=V^{-1}$, what is the result of $V \cdot V^T$ in this case? It's an identity matrix or to oversimplify it, it is $1$.

So, let's take off the parenthesis and start removing all the $VV^T$

$Cov(Y)=  V^TV \Lambda  \cdot V^TV=\Lambda$
...
Didn't we say that the covariance of Y is $Cov(Y)= \frac{1}{n-1}Y^TY= V^TV \Lambda  \cdot V^TV=\Lambda$
So then we replace 
